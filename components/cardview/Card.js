...
const imageLoader = ({ src, width, quality }) => {
    return `http://image.goodchoice.kr/resize_490x348/affiliate/2019/06/12/${src}?w=${width}&q=${quality || 75}`
}

const RatingScorePer = (score) => {
    return String(score * 20.00) + '%'
}
                    ...
                    <div className={styles.placeListScore_container}>
                        <div className={styles.placeListScore_rating}>
                            <span className={styles.placeListScore_ratingStars}>
                                <i className={styles.placeListScore_stars} 
                                    style={{width: RatingScorePer(props.ratingScoreAvg)}}></i>
